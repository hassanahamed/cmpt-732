...
def fas():
    for i in range(4096667):
        address = 'delhi'
        geolocator = Nominatim(user_agent='cmpt-732')
        location = geolocator.geocode(address, exactly_one=True,language="english", addressdetails=True)
        if location and location.raw and location.raw['address'] and location.raw['address']['country']:
